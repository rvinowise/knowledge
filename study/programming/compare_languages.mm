<map version="freeplane 1.9.13">
<!--To view this file, download free mind mapping software Freeplane from https://www.freeplane.org -->
<attribute_registry SHOW_ATTRIBUTES="selected">
    <attribute_name MANUAL="true" NAME="src">
        <attribute_value VALUE=""/>
        <attribute_value VALUE="cottee"/>
        <attribute_value VALUE="https://www.youtube.com/shorts/66t2mLXtj9E"/>
        <attribute_value VALUE="https://www.youtube.com/shorts/66t2mLXtj9E?&amp;ab_channel=THESTRAIGHTPATH"/>
    </attribute_name>
</attribute_registry>
<node TEXT="compare_languages" LOCALIZED_STYLE_REF="AutomaticLayout.level.root" FOLDED="false" ID="ID_696401721" CREATED="1610381621824" MODIFIED="1656835963214"><hook NAME="MapStyle" zoom="1.2">
    <properties show_icon_for_attributes="true" edgeColorConfiguration="#ff3333ff,#33ff00ff,#ffcc33ff" show_notes_in_map="false" show_note_icons="false" fit_to_viewport="false"/>

<map_styles>
<stylenode LOCALIZED_TEXT="styles.root_node" STYLE="oval" UNIFORM_SHAPE="true" VGAP_QUANTITY="24 pt">
<font SIZE="24"/>
<edge COLOR="#000000"/>
<stylenode LOCALIZED_TEXT="styles.predefined" POSITION="right" STYLE="bubble">
<edge COLOR="#000000"/>
<stylenode LOCALIZED_TEXT="default" ID="ID_271890427" ICON_SIZE="12 pt" FORMAT_AS_HYPERLINK="false" COLOR="#000000" STYLE="fork" NUMBERED="false" FORMAT="STANDARD_FORMAT" TEXT_ALIGN="DEFAULT" BORDER_WIDTH_LIKE_EDGE="false" BORDER_WIDTH="1 px" BORDER_COLOR_LIKE_EDGE="false" BORDER_COLOR="#808080" BORDER_DASH_LIKE_EDGE="false" BORDER_DASH="SOLID" VGAP_QUANTITY="2 pt" MAX_WIDTH="10 cm" MIN_WIDTH="0 cm">
<arrowlink SHAPE="CUBIC_CURVE" COLOR="#000000" WIDTH="2" TRANSPARENCY="200" DASH="" FONT_SIZE="9" FONT_FAMILY="SansSerif" DESTINATION="ID_271890427" STARTARROW="DEFAULT" ENDARROW="NONE"/>
<font NAME="Tahoma" SIZE="10" BOLD="false" STRIKETHROUGH="false" ITALIC="false"/>
<edge STYLE="bezier" COLOR="#000000" WIDTH="1" DASH="SOLID"/>
<richcontent TYPE="NOTE" CONTENT-TYPE="plain/html"/>
<richcontent CONTENT-TYPE="plain/html" TYPE="DETAILS"/>
</stylenode>
<stylenode LOCALIZED_TEXT="defaultstyle.details">
<edge COLOR="#000000"/>
</stylenode>
<stylenode LOCALIZED_TEXT="defaultstyle.attributes" STYLE="fork" BORDER_WIDTH="2 px">
<font NAME="Consolas" SIZE="10" BOLD="false" STRIKETHROUGH="false"/>
<edge COLOR="#000000"/>
</stylenode>
<stylenode LOCALIZED_TEXT="defaultstyle.note" COLOR="#000000" BACKGROUND_COLOR="#ffffff" TEXT_ALIGN="LEFT">
<edge COLOR="#000000"/>
</stylenode>
<stylenode LOCALIZED_TEXT="defaultstyle.floating">
<edge STYLE="hide_edge" COLOR="#000000"/>
<cloud COLOR="#f0f0f0" SHAPE="ROUND_RECT"/>
</stylenode>
<stylenode LOCALIZED_TEXT="defaultstyle.selection"/>
</stylenode>
<stylenode LOCALIZED_TEXT="styles.user-defined" POSITION="right" STYLE="bubble">
<edge COLOR="#000000"/>
<stylenode LOCALIZED_TEXT="styles.important" ID="ID_67550811">
<icon BUILTIN="yes"/>
<arrowlink COLOR="#003399" TRANSPARENCY="255" DESTINATION="ID_67550811"/>
<edge COLOR="#000000"/>
<font NAME="Verdana"/>
</stylenode>
<stylenode TEXT="source" ID="ID_1537999715" COLOR="#7f7f7f" STYLE="bubble">
<icon BUILTIN="emoji-26F2"/>
<font BOLD="true"/>
</stylenode>
<stylenode TEXT="polemic" ID="ID_1234796638" COLOR="#7f7f7f" STYLE="bubble">
<icon BUILTIN="emoji-1F5E3"/>
<font BOLD="true"/>
</stylenode>
<stylenode TEXT="structuring hub" ID="ID_1662213094" COLOR="#7f7f7f" STYLE="bubble">
<font BOLD="true" ITALIC="false"/>
</stylenode>
<stylenode TEXT="objection" ID="ID_772028533" COLOR="#7f7f7f" STYLE="bubble">
<icon BUILTIN="closed"/>
<font BOLD="true" ITALIC="false"/>
</stylenode>
<stylenode TEXT="lacking info" ID="ID_1832303129" FORMAT_AS_HYPERLINK="false" COLOR="#666666">
<icon BUILTIN="help"/>
<font BOLD="false" ITALIC="true"/>
</stylenode>
<stylenode TEXT="against" ID="ID_1225383813">
<icon BUILTIN="button_cancel"/>
</stylenode>
<stylenode TEXT="in favour of" ID="ID_112429140">
<icon BUILTIN="button_ok"/>
</stylenode>
<stylenode TEXT="info node" ID="ID_154232785">
<icon BUILTIN="info"/>
</stylenode>
<stylenode TEXT="source node" ID="ID_1296520994">
<icon BUILTIN="emoji-26F2"/>
</stylenode>
<stylenode TEXT="speech" ID="ID_160530891">
<icon BUILTIN="emoji-1F5E3"/>
<font ITALIC="true"/>
</stylenode>
<stylenode TEXT="topic" ID="ID_756631423" COLOR="#7f7f7f" STYLE="fork">
<font BOLD="true" ITALIC="false"/>
</stylenode>
<stylenode TEXT="history event" ID="ID_1131534156" BACKGROUND_COLOR="#e5e5e5" STYLE="fork" SHAPE_HORIZONTAL_MARGIN="1 pt" SHAPE_VERTICAL_MARGIN="1 pt" NUMBERED="false" BORDER_WIDTH_LIKE_EDGE="false" BORDER_WIDTH="1 px" BORDER_COLOR_LIKE_EDGE="false" BORDER_COLOR="#cccccc" BORDER_DASH_LIKE_EDGE="false" BORDER_DASH="SOLID">
<font NAME="Noto Sans" BOLD="false"/>
<edge STYLE="bezier" COLOR="#cccccc" WIDTH="1" DASH="SOLID"/>
</stylenode>
<stylenode TEXT="date" ID="ID_612383430" COLOR="#000000" BACKGROUND_COLOR="#ffffff" STYLE="fork" BORDER_COLOR="#cccccc">
<icon BUILTIN="emoji-1F552"/>
<font BOLD="false"/>
<edge COLOR="#cccccc" WIDTH="1"/>
</stylenode>
<stylenode TEXT="wrong place" ID="ID_1192160512" COLOR="#990000">
<icon BUILTIN="help"/>
</stylenode>
<stylenode TEXT="list item" ID="ID_1531959439" NUMBERED="true"/>
<stylenode TEXT="end of my speech" BACKGROUND_COLOR="#ffffec">
<icon BUILTIN="help"/>
</stylenode>
<stylenode TEXT="--------------------------------" BACKGROUND_COLOR="#ffffff">
<font BOLD="true"/>
</stylenode>
<stylenode TEXT="alt E" ID="ID_205177288" BACKGROUND_COLOR="#ffffec">
<icon BUILTIN="help"/>
</stylenode>
<stylenode TEXT="done" COLOR="#666666" BACKGROUND_COLOR="#cccccc">
<icon BUILTIN="emoji-2B1B"/>
</stylenode>
<stylenode TEXT="action">
<icon BUILTIN="launch"/>
</stylenode>
</stylenode>
<stylenode LOCALIZED_TEXT="styles.AutomaticLayout" POSITION="right" STYLE="bubble">
<edge COLOR="#000000"/>
<stylenode LOCALIZED_TEXT="AutomaticLayout.level.root" COLOR="#ffffff" BACKGROUND_COLOR="#000000" STYLE="fork" SHAPE_HORIZONTAL_MARGIN="10 pt" SHAPE_VERTICAL_MARGIN="10 pt" BORDER_COLOR_LIKE_EDGE="false" BORDER_COLOR="#000000">
<font SIZE="10" BOLD="true"/>
</stylenode>
<stylenode LOCALIZED_TEXT="AutomaticLayout.level,1" COLOR="#7f7f7f">
<font NAME="Verdana" SIZE="16" BOLD="true"/>
<edge COLOR="#000000"/>
</stylenode>
<stylenode LOCALIZED_TEXT="AutomaticLayout.level,2" COLOR="#7f7f7f">
<font NAME="Verdana" SIZE="14" BOLD="true"/>
<edge COLOR="#000000"/>
</stylenode>
<stylenode LOCALIZED_TEXT="AutomaticLayout.level,3" COLOR="#7f7f7f">
<font NAME="Verdana" SIZE="12" BOLD="true"/>
<edge COLOR="#000000"/>
</stylenode>
</stylenode>
</stylenode>
</map_styles>
</hook>
<font NAME="Tahoma"/>
<node TEXT="type: " POSITION="left" ID="ID_1234978731" CREATED="1656835963271" MODIFIED="1656835963426"/>
<node TEXT="JVM" POSITION="right" ID="ID_681387040" CREATED="1656836355953" MODIFIED="1656836362645">
<node TEXT="" ID="ID_468438184" CREATED="1656835963427" MODIFIED="1656835969163">
<icon BUILTIN="emoji-26F2"/>
<node TEXT="comparison of java machine Ls" ID="ID_552748073" CREATED="1656835969442" MODIFIED="1656836276866" LINK="https://itsallbinary.com/java-vs-groovy-scala-kotlin-code-comparison-of-jvm-languages/"/>
</node>
<node TEXT="java" ID="ID_1814867718" CREATED="1657194900035" MODIFIED="1657194905223">
<icon BUILTIN="control/type_of_parent"/>
<node TEXT="example code snippets" ID="ID_654588684" CREATED="1657194906118" MODIFIED="1657204291683" LINK="https://www.javatips.net/api/">
<icon BUILTIN="book_open"/>
</node>
</node>
<node TEXT="kotlin" ID="ID_1375964691" CREATED="1656836381766" MODIFIED="1656931969737">
<icon BUILTIN="control/type_of_parent"/>
<node TEXT="standard for android" ID="ID_654644027" CREATED="1656836385019" MODIFIED="1656836391328">
<icon BUILTIN="good"/>
</node>
<node TEXT="? pragmatic" ID="ID_1459765819" CREATED="1656836462177" MODIFIED="1656836694331"/>
<node TEXT="course in kotlin by mindorks" ID="ID_873652132" CREATED="1657080244888" MODIFIED="1657080948880" LINK="https://blog.mindorks.com/learn-kotlin-destructuring-declarations">
<icon BUILTIN="emoji-26F2"/>
<icon BUILTIN="book_open"/>
</node>
</node>
<node TEXT="scala" ID="ID_973097272" CREATED="1656836392318" MODIFIED="1656931969737">
<icon BUILTIN="control/type_of_parent"/>
<node TEXT="overly complicated" ID="ID_106689146" CREATED="1656836394080" MODIFIED="1656836401147">
<icon BUILTIN="bad"/>
</node>
</node>
<node TEXT="groovy" ID="ID_1679876642" CREATED="1656836402716" MODIFIED="1656931969736">
<icon BUILTIN="control/type_of_parent"/>
<node TEXT="most similar to java" ID="ID_1946823895" CREATED="1656836406967" MODIFIED="1656836447868"/>
<node TEXT="less popular" ID="ID_771416599" CREATED="1656836414509" MODIFIED="1656836420540">
<icon BUILTIN="bad"/>
</node>
<node TEXT="its classes can&apos;t be used in other JVM languages" ID="ID_129280893" CREATED="1657059377203" MODIFIED="1657059581671" LINK="https://discuss.kotlinlang.org/t/extending-groovy-class-from-kotlin/1675/5">
<icon BUILTIN="bad"/>
<node TEXT="but groovy itself can use classes from other JVM languages" ID="ID_1488745239" CREATED="1657059447925" MODIFIED="1657059597541">
<icon BUILTIN="good"/>
</node>
</node>
</node>
<node TEXT="clojure" ID="ID_223515246" CREATED="1656836432444" MODIFIED="1656931969731">
<icon BUILTIN="control/type_of_parent"/>
</node>
<node TEXT="as a project building DSL" ID="ID_273891521" CREATED="1656931899616" MODIFIED="1656931930934">
<node TEXT="groovy vs kotlin comparison" ID="ID_552517326" CREATED="1656931933701" MODIFIED="1656931943567" LINK="https://tomgregory.com/5-reasons-to-switch-to-the-gradle-kotlin-dsl/"/>
</node>
</node>
<node TEXT="controvercies" POSITION="right" ID="ID_1304683830" CREATED="1656933061996" MODIFIED="1656933068057">
<node TEXT="typing" ID="ID_827497977" CREATED="1656933068194" MODIFIED="1656933071288">
<node TEXT="static" ID="ID_321100709" CREATED="1656933071429" MODIFIED="1656933215463">
<icon BUILTIN="control/type_of_parent"/>
<node TEXT="kotlin" ID="ID_1492788635" CREATED="1656933092666" MODIFIED="1656933260402">
<icon BUILTIN="present"/>
</node>
<node TEXT="cpp" ID="ID_1379388695" CREATED="1656933098195" MODIFIED="1656933369761">
<icon BUILTIN="present"/>
</node>
<node TEXT="c#" ID="ID_1851218979" CREATED="1656933235676" MODIFIED="1656933260391">
<icon BUILTIN="present"/>
</node>
<node TEXT="java" ID="ID_980019687" CREATED="1656933102828" MODIFIED="1656933260383">
<icon BUILTIN="present"/>
</node>
<node TEXT="stable code" ID="ID_1979903453" CREATED="1656933158089" MODIFIED="1656933209216">
<icon BUILTIN="good"/>
</node>
</node>
<node TEXT="dynamic" ID="ID_1511436099" CREATED="1656933073447" MODIFIED="1656933215455">
<icon BUILTIN="control/type_of_parent"/>
<node TEXT="python" LOCALIZED_STYLE_REF="default" ID="ID_1756217174" CREATED="1656933077496" MODIFIED="1656933266861" COLOR="#000000" BACKGROUND_COLOR="#ffffff">
<icon BUILTIN="present"/>
</node>
<node TEXT="groovy" LOCALIZED_STYLE_REF="default" ID="ID_1650051863" CREATED="1656933080332" MODIFIED="1656933266867" COLOR="#000000" BACKGROUND_COLOR="#ffffff">
<icon BUILTIN="present"/>
</node>
<node TEXT="convenient programming" ID="ID_882078929" CREATED="1656933177546" MODIFIED="1656933211804">
<icon BUILTIN="good"/>
</node>
</node>
</node>
<node TEXT="garbage collection" ID="ID_1863106917" CREATED="1656933283153" MODIFIED="1656933296004">
<node TEXT="automatic GC" ID="ID_981591885" CREATED="1656933296098" MODIFIED="1656933482964">
<icon BUILTIN="control/type_of_parent"/>
<node TEXT="c#" ID="ID_1046127870" CREATED="1656933319543" MODIFIED="1657059365696">
<icon BUILTIN="present"/>
</node>
<node TEXT="go" ID="ID_446881170" CREATED="1656933323867" MODIFIED="1657059365697">
<icon BUILTIN="present"/>
</node>
<node TEXT="java" ID="ID_1645443461" CREATED="1656933327916" MODIFIED="1657059365423">
<icon BUILTIN="present"/>
</node>
<node TEXT="convenient programming" ID="ID_616485074" CREATED="1656933399775" MODIFIED="1656933407934">
<icon BUILTIN="good"/>
</node>
<node TEXT="non-memory recources need to be managed separately" ID="ID_1276716038" CREATED="1656933412529" MODIFIED="1656933447090">
<icon BUILTIN="bad"/>
</node>
</node>
<node TEXT="no GC" ID="ID_1738463228" CREATED="1656933331112" MODIFIED="1656933482970">
<icon BUILTIN="control/type_of_parent"/>
<node TEXT="cpp" ID="ID_1533148753" CREATED="1656933342179" MODIFIED="1657059365698">
<icon BUILTIN="present"/>
</node>
<node TEXT="all resources are managed, not only memory" ID="ID_751292316" CREATED="1656933383841" MODIFIED="1656933398312">
<icon BUILTIN="good"/>
</node>
<node TEXT="error prone" ID="ID_1075157358" CREATED="1656933452050" MODIFIED="1656933462521">
<icon BUILTIN="bad"/>
</node>
</node>
</node>
<node TEXT="database interface" ID="ID_14449631" CREATED="1657129637925" MODIFIED="1657129653927">
<node TEXT="ORM" ID="ID_147885562" CREATED="1657129654115" MODIFIED="1657129782367">
<icon BUILTIN="control/type_of_parent"/>
<node TEXT="is considered standard by many" ID="ID_267682418" CREATED="1657129798720" MODIFIED="1657129820218">
<icon BUILTIN="good"/>
</node>
<node TEXT="against" ID="ID_643002071" CREATED="1657132490898" MODIFIED="1657144008888">
<icon BUILTIN="bad"/>
<node TEXT="testimonies on stackoverflow" ID="ID_1994302364" CREATED="1657129826080" MODIFIED="1657129860017" LINK="https://stackoverflow.com/questions/494816/using-an-orm-or-plain-sql"/>
<node TEXT="yegor bugayenko" ID="ID_1132576710" CREATED="1657132507384" MODIFIED="1657138487056" LINK="https://www.yegor256.com/2014/12/01/orm-offensive-anti-pattern.html">
<node TEXT="he&apos;s not arguing for plain sql, but rather for his implementation of ORM" ID="ID_689945244" CREATED="1657138490034" MODIFIED="1657138526351">
<icon BUILTIN="bad"/>
</node>
</node>
<node TEXT="detailed explanation of complexity issues of ORM, bad performance" ID="ID_1445512967" CREATED="1657149560708" MODIFIED="1657149623875" LINK="https://dev.solita.fi/2021/06/01/why-avoid-an-orm.html"/>
</node>
<node TEXT="use for operations apart from &quot;read&quot; (update,insert,delete)" ID="ID_1751908405" CREATED="1657143397961" MODIFIED="1657143455797" LINK="https://stackoverflow.com/a/494871/7513754">
<icon BUILTIN="good"/>
</node>
<node TEXT="" ID="ID_900513049" CREATED="1657143860729" MODIFIED="1657143860729"/>
</node>
<node TEXT="DAL" ID="ID_381129938" CREATED="1657142087735" MODIFIED="1657142091333">
<icon BUILTIN="control/type_of_parent"/>
<node TEXT="thin layer over plain sql" ID="ID_822518844" CREATED="1657142092038" MODIFIED="1657142102283">
<icon BUILTIN="box_inscribed"/>
</node>
<node TEXT="something between ORM and plain SQL" ID="ID_993823949" CREATED="1657142104419" MODIFIED="1657142120267">
<icon BUILTIN="box_inscribed"/>
</node>
</node>
<node TEXT="plain sql" ID="ID_486880716" CREATED="1657129658669" MODIFIED="1657129782374">
<icon BUILTIN="control/type_of_parent"/>
<node TEXT="faster" ID="ID_1747680372" CREATED="1657129791106" MODIFIED="1657129794707">
<icon BUILTIN="good"/>
</node>
<node TEXT="use for operation &quot;read&quot;" ID="ID_1296875763" CREATED="1657143369792" MODIFIED="1657143396513">
<icon BUILTIN="good"/>
</node>
<node TEXT="against" ID="ID_1095403057" CREATED="1657143961469" MODIFIED="1657143965521">
<icon BUILTIN="bad"/>
<node TEXT="problems with plain sql" LOCALIZED_STYLE_REF="default" ID="ID_176992687" CREATED="1657143864503" MODIFIED="1657143933405" LINK="https://github.com/keredson/DKO" COLOR="#000000" BACKGROUND_COLOR="#ffffff">
<node TEXT="proponents of a DAL (keredson)" ID="ID_1079843526" CREATED="1657143925427" MODIFIED="1657143927775">
<icon BUILTIN="emoji-26F2"/>
</node>
</node>
</node>
</node>
<node TEXT="esoteric solutions" ID="ID_1669909597" CREATED="1657191309051" MODIFIED="1657191453812">
<icon BUILTIN="control/type_of_parent"/>
<node TEXT="edgedb" ID="ID_1716934141" CREATED="1657191397322" MODIFIED="1657191410894">
<node TEXT="analysis of drawback of ORMs and plain SQL" ID="ID_1108504705" CREATED="1657191411271" MODIFIED="1657191428354" LINK="https://www.edgedb.com/blog/a-solution-to-the-sql-vs-orm-dilemma"/>
</node>
</node>
</node>
</node>
<node TEXT="variables" POSITION="left" ID="ID_1297806066" CREATED="1656835993741" MODIFIED="1656835998685">
<node TEXT="L" ID="ID_1910314962" CREATED="1656835999839" MODIFIED="1656836002474">
<node TEXT="language" ID="ID_1604329720" CREATED="1656836002665" MODIFIED="1656836004967"/>
</node>
<node TEXT="JVM" ID="ID_1423293958" CREATED="1656836368083" MODIFIED="1656836371229">
<node TEXT="Java virtual machine" ID="ID_132489908" CREATED="1656836371471" MODIFIED="1657059532346"/>
</node>
<node TEXT="DSL" ID="ID_81658466" CREATED="1656931953732" MODIFIED="1656931955013">
<node TEXT="domain specific languagw" ID="ID_1014317715" CREATED="1656931955317" MODIFIED="1656931960373"/>
</node>
<node TEXT="GC" ID="ID_1380618294" CREATED="1656933306876" MODIFIED="1656933308179">
<node TEXT="garbage collector" ID="ID_1561144717" CREATED="1656933308794" MODIFIED="1656933313139"/>
</node>
<node TEXT="ORM" ID="ID_1067734108" CREATED="1657129666297" MODIFIED="1657129668038">
<node TEXT="object relation model" ID="ID_1997276782" CREATED="1657129668821" MODIFIED="1657129677907">
<node TEXT="the layer linking a database to the user application" ID="ID_189641628" CREATED="1657129678315" MODIFIED="1657129740232"/>
</node>
</node>
<node TEXT="DAL" ID="ID_1245372359" CREATED="1657142072550" MODIFIED="1657142074304">
<node TEXT="data access language" ID="ID_440331541" CREATED="1657142074918" MODIFIED="1657142079425"/>
</node>
</node>
</node>
</map>
